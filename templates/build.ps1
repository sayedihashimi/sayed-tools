...
[System.IO.FileInfo]$slnfile = (join-path $scriptDir 'tool-src\Mutant.Chicken.Tools.sln')
[System.IO.FileInfo[]]$csProjects = (Join-Path $scriptDir 'tool-src\CoverageConverter\CoverageConverter.csproj')
...
function GetExistingVersion{
    ...
    param(
        ...
        $nuspecFile = (Join-Path $scriptDir 'sayed-tools.nuspec')
    )
    process{
